...
func movement_factory() -> Node:
	if (Player.state.airborne.on_ceiling and Player.state.movement == Enums.Movement.GRIP):
		return CeilingMovement.new(self.character)
	elif (Player.state.airborne.on_wall and Player.state.movement == Enums.Movement.GRIP):
